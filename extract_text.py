        ...
        if file_ext in ["jpeg", "jpg", "png"]:
            ...
            for attempt in range(2):
                ...
                for chunk in chunk_text_for_streaming(ocr_text):
                    ...
                    math_instruction = "Use proper mathematical symbols in plain text (e.g., 5/6 instead of LaTeX \\frac{5}{6})." if math_present else ""

                    if action == "summarize":
                        prompt = f"Summarize this text clearly and concisely. {math_instruction}\n{chunk}"
                    elif action == "custom":
                        if not custom_prompt:
                            yield "⚠️ No custom prompt provided.\n"
                            return
                        if any(word in custom_prompt.lower() for word in ["solve", "calculate", "answer"]):
                            prompt = f"Solve the following problem clearly and concisely. {math_instruction}\n{chunk}\nCustom instructions:\n{custom_prompt}"
                        else:
